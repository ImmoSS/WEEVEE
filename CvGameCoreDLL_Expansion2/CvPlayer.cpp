...
void CvPlayer::acquireCity(CvCity* pOldCity, bool bConquest, bool bGift)
{
	...
	if(bConquest)
	{
			...
			// Will this be the first time anyone on the team has owned this city? And was it not a city state?
	//if(GC.getGame().isOption("GAMEOPTION_SPOILS_OF_WAR")) // Here we check for the Spoils of War Game option to be enabled
	//{
		bool bTeamHasCaptured = false;
		for (int i=0; i < MAX_PLAYERS; i++)
		{
			PlayerTypes ePlayer = (PlayerTypes)i;
			if (GET_PLAYER(ePlayer).getTeam() == getTeam() && pOldCity->isEverOwned(ePlayer))
			{
				bTeamHasCaptured = true;
				break;
			}
		}
		if (!bTeamHasCaptured && !GET_PLAYER(pOldCity->getOriginalOwner()).isMinorCiv())
		{
...
{
	int iScore = getTotalLand();

	if (GC.getGame().isOption("GAMEOPTION_TWEAKED_SCORING"))
		iScore *= GC.getNEW_SCORE_LAND_MULTIPLIER();
	else 
		iScore *= GC.getSCORE_LAND_MULTIPLIER();
...
#endif
		if (GC.getGame().isOption("GAMEOPTION_TWEAKED_SCORING"))
			iScore += pGameReligions->GetNumCitiesFollowing(eReligion) * GC.getNEW_SCORE_RELIGION_CITIES_MULTIPLIER();
		else
