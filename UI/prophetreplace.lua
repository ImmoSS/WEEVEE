            ...
            for i = 0, GameDefines.MAX_MAJOR_CIVS - 1, 1 do
                if (iPlayer ~= i) then
                    ...
                    if (pOtPlayer ~= nil) and pOtPlayer:IsEverAlive() and pOtPlayer:IsAlive() and (pOtPlayer:GetCapitalCity() ~= nil) and player:GetTeam() == pOtPlayer:GetTeam() then
                        ...
                        iNumToSet = (iNumToSet +  math.floor(otherCapital:GetBaseJONSCulturePerTurn() * .25) )
                    end
                end
            end
...
local iCiv = GameInfoTypes["CIVILIZATION_BABYLON"]
local bIsActive = JFD_IsCivilisationActive(iCiv)

function BabyDegenBuildingKek(iPlayer)
	local pPlayer = Players[iPlayer]
	if pPlayer:GetCivilizationType() == GameInfoTypes["CIVILIZATION_BABYLON"] then
		for pCity in pPlayer:Cities() do
			local cityPopulation = pCity:GetPopulation()
			if pCity:IsHasBuilding(GameInfoTypes.BUILDING_BABYLON_DEGEN) and cityPopulation <= 10 then
					pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_FULL_DEGEN_TRAIT, 1)
			else
				pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_FULL_DEGEN_TRAIT, 0)
			end
		end
	end
end
...
if bIsActive then
	GameEvents.PlayerDoTurn.Add(BabyDegenBuildingKek)
	GameEvents.CityConstructed.Add(BabyDegenBuildingKek)
end
...
local OldUnitType = "UNIT_WORKER"
local oldUnitID = GameInfo.Units.UNIT_WORKER.ID
local newUnitID = GameInfo.Units.UNIT_AKKAD_WORKER.ID
local iCivType = GameInfo.Civilizations["CIVILIZATION_LITE_AKKAD"].ID

function AkkadWorkerOverride(iPlayer, iUnit)
    local pPlayer = Players[iPlayer];
    if (pPlayer:IsEverAlive()) then
        if (pPlayer:GetCivilizationType() == iCivType) then
       	    if pPlayer:GetUnitByID(iUnit) ~= nil then
...
Events.SerialEventUnitCreated.Add(AkkadWorkerOverride)


-- Now reverse!! I typed this backwards so you will get a better explantion below in similar functions but right now I couldnt be bothered copy-pasting so I decided instead, to write this novel. ~EAP

local OldUnitType = "UNIT_AKKAD_WORKER"
local oldUnitID = GameInfo.Units.UNIT_AKKAD_WORKER.ID
local newUnitID = GameInfo.Units.UNIT_WORKER.ID
local iCivType = GameInfo.Civilizations["CIVILIZATION_LITE_AKKAD"].ID

function AkkadWorkerROverride(iPlayer, iUnit)
    local pPlayer = Players[iPlayer];
    if (pPlayer:IsEverAlive()) then
        if (not pPlayer:GetCivilizationType() == iCivType) then
       	    if pPlayer:GetUnitByID(iUnit) ~= nil then
...
Events.SerialEventUnitCreated.Add(AkkadWorkerROverride)

----////--------------------------------------------------
				--- EXPLO WORKBOAT ---
--- This replaces your newly created workboats with Chile workboats if you have exploration opened. ---
----////--------------------------------------------------
...
function ChileWorkBoatOverride(iPlayer, iUnit)
    local pPlayer = Players[iPlayer];
    if (pPlayer:IsEverAlive()) then
        if (pPlayer:HasPolicy(GameInfo.Policies["POLICY_EXPLORATION"].ID)) then
       	    if pPlayer:GetUnitByID(iUnit) ~= nil then
				pUnit = pPlayer:GetUnitByID(iUnit);
                if (pUnit:GetUnitType() == oldUnitID) then
                    local newUnit = pPlayer:InitUnit(newUnitID, pUnit:GetX(), pUnit:GetY())
...
function ChileWorkBoatROverride(iPlayer, iUnit)
    local pPlayer = Players[iPlayer];
    if (pPlayer:IsEverAlive()) then
        if (not pPlayer:HasPolicy(GameInfo.Policies["POLICY_EXPLORATION"].ID)) then
       	    if pPlayer:GetUnitByID(iUnit) ~= nil then
		pUnit = pPlayer:GetUnitByID(iUnit);
                if (pUnit:GetUnitType() == oldUnitID) then
                    local newUnit = pPlayer:InitUnit(newUnitID, pUnit:GetX(), pUnit:GetY())
...
-- Cultural Centers Updates
-- Author: Glossen
--DateCreated: 10/4/2023 11:30:ish PM
--------------------------------------------------------------
function Cultural_Centers_OnPolicyAdopted(playerID, policyID)

	local player = Players[playerID]
	local hasCultural_Centers = (policyID==GameInfo.Policies["POLICY_CULTURAL_CENTERS"].ID)

	if	(hasCultural_Centers) then
		local pCity = player:GetCapitalCity();
		pCity:SetNumRealBuilding(GameInfoTypes["BUILDING_CULTURAL_CENTERS"], 1)
	end
end
GameEvents.PlayerAdoptPolicy.Add(Cultural_Centers_OnPolicyAdopted);
...
function Alternative_Energy_OnPolicyAdopted(playerID, policyID)
	...
	if (policyID == GameInfo.Policies["POLICY_ALTERNATIVE_ENERGY"].ID) then 
		local pCity = player:GetCapitalCity(); 
		pCity:SetNumRealBuilding(GameInfoTypes["BUILDING_ALTERNATIVE_URANIUM"], 1);
	end
end
...
function Honor_AddGlobalHappinessToNationalWonders_PlayerDoTurn(iPlayer)
	local nationalWonders = {47, 48, 49, 50, 51, 52, 53, 54, 113, 114, 124};

	local player = Players[iPlayer]
	local cities = player:Cities();

	if (player:IsEverAlive() and player:HasPolicy(GameInfo.Policies["POLICY_MILITARY_CASTE"].ID) and
		player:HasPolicy(GameInfo.Policies["POLICY_MILITARY_TRADITION"].ID) and
		player:HasPolicy(GameInfo.Policies["POLICY_DISCIPLINE"].ID)) then
		local nwCount = 0;
		
		for i=1, 11, 1 do
			if(player:GetBuildingClassCount(nationalWonders[i]) == 1) then
				nwCount = nwCount + 1;
			end
		end
		local pCity = player:GetCapitalCity();
		if nwCount>player:GetBuildingClassCount(9999) then
			pCity:SetNumRealBuilding(GameInfoTypes["BUILDING_HONOR_GLOBAL_HAPPY"], nwCount)
		end		
	end
	
end
GameEvents.PlayerDoTurn.Add(Honor_AddGlobalHappinessToNationalWonders_PlayerDoTurn);
...
--[[ four year plan (megawac)
function FourYearPlanBuildingConstructed(playerId, cityId)
	print('FourYearPlan: Building constructed');
	local player = Players[playerId];
	local city = player:GetCityByID(cityID);
	if city:IsHasBuilding(GameInfoTypes["BUILDING_FOUR_YEAR_PLAN_ACTIVE"]) then
...
function FourYearPlanCheckEra(playerId)
	print('FourYearPlan: Era change detected');
	local player = Players[playerId];
	for city in player:Cities() do
		city:SetNumRealBuilding(GameInfoTypes["BUILDING_FOUR_YEAR_PLAN_ACTIVE"], 0);
	end
end

function FourYearPlanPolicyTaken(playerId, policyId)
	print('FourYearPlan: Policy taken');
	local player = Players[playerId];
	if (policyId == GameInfo.Policies["POLICY_MOBILIZATION"].ID) then
		for city in player:Cities() do
			city:SetNumRealBuilding(GameInfoTypes["BUILDING_FOUR_YEAR_PLAN_ACTIVE"], 1);
		end
	end
end

print('FourYearPlan: adding events');
GameEvents.CityConstructed.Add(FourYearPlanBuildingConstructed);
GameEvents.TeamSetEra.Add(FourYearPlanCheckEra);
GameEvents.PlayerAdoptPolicy.Add(FourYearPlanPolicyTaken);
print('FourYearPlan: events added'); --]]

local civOtto = GameInfoTypes["CIVILIZATION_OTTOMAN"]
local ottoIsActive = JFD_IsCivilisationActive(civOtto)	
function OttoBuildingFaith(playerID, cityID)
	local player = Players[playerID];
	local rewardFaith = 5;
	if player:GetCivilizationType() == civOtto and player:IsAlive() then
...
GameEvents.CityConstructed.Add(OttoBuildingFaith)
